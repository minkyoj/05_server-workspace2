...
<body>
    ...
    <P>
        ...
        우리가 기존에 a태그로 요청 form submit 요청방식은 "동기식 요청" 이었음 => 응답페이지가 돌아와야만 볼 수 있음(페이지 깜빡거림)

        * 동기식 / 비동기식
        > 동기식 (a, form submit)
        <ul>
            <li>요청 처리 후 그에 해당하는 응답페이지가 돌아와야만 그 다음 작업 가능</li>
            <li>서버에 요청한 결과까지의 시간이 지연되면 무작정 계속 기다려야 함(흰 페이지로 보여질꺼임)</li>
        ...
        <ul>
            ...
            <li>요청을 보내놓고 그에 해당하는 응답(데이터)이 돌아올 때 까지 현재페이지에서 다른 작업을 할 수 있음</li>
            <li>페이지가 깜빡거리지 않음</li>
        </ul>
        ...
        * 비동기식 단점 <br>
        - 현재페이지에 지속적으로 리소스가 쌓임 => 페이지가 느려질 수 있음
        - 에러 발생시 디버깅 어려움 <br>
    ...
    <script>
        $(function(){
            ...
            $("#btn1").click(function(){
                // 기존의 동기식 통신
                // location.href='jqAjax.do?input=xxx';

                // 비동기식 통신
                $.ajax({
                    // 요청보내기
                    url : "jqAjax1.do", // 어느 url로 보낼건지
                    data : {input : $("#input1").val()},         // 보낼데이터 => 키:벨류 세트로 보내야함
                    type : "get", // 요청방식 지정
